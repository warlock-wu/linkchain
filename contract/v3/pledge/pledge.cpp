...
public:
    ...
    const char* version(){return "v2.2";}
...
(changeDeposit)(requestWithdraw)(version)(setPeriod)(getPeriod)(getDepositTime)(requestQuit))
...
void Pledge::requestQuit(){
    const char* info = "requestQuit";
    tc::Address sender = tc::App::getInstance()->sender();
    auto s = winElectorsAddress.get();
    TC_RequireWithMsg(s.find(sender) != s.end(), "Not a win node address");
    TC_Log1(sender.toString(), info);
}
